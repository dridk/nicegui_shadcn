...
def Header():
    ui.element().classes("w-full h-16 bg-red-500 ")


def Menu():


    with ui.element().classes("h-14 w-full "):
        with ui.column().classes("w-full h-full pr-3 pl-3 pt-2"):
            data = ["sacha@chu-brest.fr"]
            ui.select(data, value=data[0]).props("dense outlined").classes("w-full")

        ui.separator().classes("absolute")

    for i in range(5):
        with ui.element().classes("w-full flex justify-center"):
            with ui.button().props("color=black").classes("w-full ml-3 mr-3"):
                with ui.row():
                    ui.icon("home").style()
                    ui.label("Matthieu").style()
                    ui.label("9").style()
...
def Mail():
    ...
    with ui.element().classes("w-full flex justify-center"):
        with ui.row().classes("ml-2 mt-1 mr-2 w-full"):
            ui.label("Inbox").classes("font-bold text-lg ml-4")
            ui.toggle(['All mail', 'Unread']).props("no-caps size=md").classes("ml-auto")

    ui.separator().classes("w-full")

    with ui.element().classes("w-full flex justify-center"):
        ui.input("Search...").props("dense outlined").classes("ml-2 mr-2 w-full")


    for i in range(3): 
        
        with ui.element().classes("border rounded p-4 flex justify-center ml-2 mr-2 hover:bg-gray-100") :
            with ui.grid(columns=2).classes("w-full").style("gap:0"):
                ui.label("William Smith").classes("font-bold flex justify-start")
                ui.label("10 months ago").classes("ml-auto flex justify-end")
                ui.label("Meeting Tomorrow").classes("font-semi-bold")
            ui.label("Aperçu du mail...").classes("p-2")
            with ui.row().classes():
    ...
    # #             ui.toggle(["All mail", "Unread"]).props("flat size=sm").classes("")
    # #         ui.separator().classes()


    # with ui.row().classes("w-full mt-1 flex"):
    ...
    # ui.input(placeholder="Search...").props("outlined").classes(" m-2")
    # # with ui.row().classes("w-full"):
...
def ReadMail():
    ...
    with ui.row().classes("w-full mt-[14.5px]"):
    ...
    with ui.row().classes("w-full"):
        ui.label("WS").classes("ml-4").style("padding: 10px; border-radius:50px; background-color: hsl(240,5%,96%)")
    ...
    with ui.column().classes("w-full pl-3 pr-3"):
        with ui.element().classes("w-full"):
            ui.textarea(placeholder="Reply William Smith...").props("outlined").classes("flex justify-center")
        with ui.row().classes("w-full"):
            ui.switch("Mute this thread").props("color=black").classes("")
            ui.button("Send").props("color=slate-900 no-caps").classes("ml-auto")
    
    # with ui.element().classes("ml-4 w-[96.45%]"):
    #     ui.textarea(placeholder="Reply William Smith...").props("outlined").classes("w-50px flex justify-center")
    # with ui.row().classes("w-full"):
    #     ui.switch("Mute this thread").props("color=black").classes("ml-1")
    #     ui.button("Send").props("color=slate-900 no-caps").classes("ml-auto mr-4")
...
def page():
    ...
    with ui.element().classes("bg-white w-screen h-screen m-0"):
        with ui.splitter(limits=(16, 16), value=16).classes("h-full") as splitter:
            with splitter.before:
                Menu()
            with splitter.separator:
                Separator()
            with splitter.after:
                with ui.splitter(limits=(30, 30), value=30).classes("h-full") as splitter2:
